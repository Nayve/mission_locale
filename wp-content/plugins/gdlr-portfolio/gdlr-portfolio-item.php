		...
		function gdlr_get_portfolio_info( $array = array(), $option = array(), $wrapper = true ){
			...
			foreach($array as $post_info){	
				switch( $post_info ){
					...
					case 'type':
						if(empty($option['type'])) break;
						$ret .= '<div class="portfolio-info portfolio-type">';
						$ret .= '<span class="info-head gdlr-title">';
						// $ret .= '<i class="fa ' . gdlr_fa_class('icon-clock-à') . '" ></i>';
						$ret .= $option['type'];		
						$ret .= ' </span>';
						$ret .= '</div>';
						break;
					...
					case 'date':
						if(empty($option['start-date'])) break;

						$start_date = strtotime($option['start-date'] . ' 00:00:00');
						...
						$ret .= ' </span>';
						$ret .= '<br>';	
						if (!empty($option['end-date'])) {
							$ret .= 'Du ';	
						} else {
							$ret .= 'Le ';
						}
						$ret .= '<span class="activity-date">';
						$ret .= date_i18n($theme_option['date-format'], $start_date);
						$ret .= ' </span>';	
						if (!empty($option['end-date'])) {
							$ret .= '<br>';	
							$ret .= ' au ';
							$ret .= '<span class="activity-date">';
							$ret .= date_i18n($theme_option['date-format'], strtotime($option['end-date'] . ' 00:00:00'));
							$ret .= ' </span>';	
						} 
						$ret .= '</div>';
						break;
					...
					case 'location':
						if(empty($option['location-street']) || empty($option['location-city'])) break;
						$ret .= '<div class="portfolio-info portfolio-location">';
						$ret .= '<span class="info-head gdlr-title">';
						$ret .= '<i class="fa ' . gdlr_fa_class('icon-map-marker') . '" ></i>';
						$ret .= 'Où ?';
						$ret .= ' </span>';
						if(!empty($option['location-organization'])) {
							$ret .= '<br>';	
							$ret .= $option['location-organization'];						
						}
						$ret .= '<br>';	
						$ret .= $option['location-street'];						
						$ret .= '<br>';	
						$ret .= $option['location-city'];	
						$ret .= '</div>';
						break;
					...
					// 	$ret .= '<span class="info-head gdlr-title">' . __('Compétences requises', 'gdlr-portfolio') . ' </span>';
					// 	$ret .= '<br>';	
					// 	$ret .= $tag;						
					// 	$ret .= '</div>';						
					// 	break;		
					...
					case 'info': 
						if(empty($option['info-date-1']) && empty($option['info-date-2']) && empty($option['info-date-3'])) break;
						$ret .= '<div class="portfolio-info portfolio-info">';
						$ret .= '<span class="info-head gdlr-title">';
						$ret .= '<i class="fa ' . gdlr_fa_class('icon-info-circle') . '" ></i>';
						$ret .= 'Séance(s) d\'information';
						$ret .= ' </span>';
						$ret .= ' <ul>';

						if (!empty($option['info-date-1'])) {
							$ret .= '<li>';	
							$ret .= 'le ';	
							$ret .= date_i18n($theme_option['date-format'], strtotime($option['info-date-1'] . ' 00:00:00'));
							if (!empty($option['info-time-1'])) {
								$ret .= ' à ';
								$ret .= gdlr_session_time_conversion($option['info-time-1']);		
							}
							$ret .= '</li>';	
						}	

						if (!empty($option['info-date-2'])) {
							$ret .= '<li>';		
							$ret .= 'le ';	
							$ret .= date_i18n($theme_option['date-format'], strtotime($option['info-date-2'] . ' 00:00:00'));
							if (!empty($option['info-time-1'])) {
								$ret .= ' à ';
								$ret .= gdlr_session_time_conversion($option['info-time-1']);		
							}
						...
						if (!empty($option['info-date-3'])) {
							$ret .= '<li>';		
							$ret .= 'le ';	
							$ret .= date_i18n($theme_option['date-format'], strtotime($option['info-date-3'] . ' 00:00:00'));
							if (!empty($option['info-time-1'])) {
								$ret .= ' à ';
								$ret .= gdlr_session_time_conversion($option['info-time-1']);		
							}
					...
					case 'contact':
						if(empty($option['contact-1']) || empty($option['contact-2'])) break;
						$ret .= '<div class="portfolio-info portfolio-contact">';
						$ret .= '<span class="info-head gdlr-title">';
						$ret .= '<i class="fa ' . gdlr_fa_class('icon-question-circle') . '" ></i>';
						$ret .= 'Pour toute question ou renseignement';		
						$ret .= ' </span>';
						$ret .= ' <ul>';

						if (!empty($option['contact-1'])) {
							$ret .= '<li>';	
							$ret .= $option['contact-1'];
							$ret .= '</li>';	
						}	

						if (!empty($option['contact-2'])) {
							$ret .= '<li>';	
							$ret .= $option['contact-2'];
							$ret .= '</li>';	
						}	
	...
	if( !function_exists('gdlr_get_modern_portfolio') ){
		function gdlr_get_modern_portfolio($query, $size, $thumbnail_size, $layout = 'fitRows'){
			...
			while($query->have_posts()){ $query->the_post();
				...
				$ret .= '<div class="gdlr-ux gdlr-modern-portfolio-ux koala">';
				...
				$test = gdlr_get_portfolio_info(array('date'), $port_option, false);
				$ret .= '<div class="gdlr-portfolio-info gdlr-info-font">' . $test . '</div>'; 
